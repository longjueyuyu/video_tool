...
class VideoTrimmerPro:
    ...
    def create_widgets(self):
        ...
        self.tab_control.add(self.subtitle_tab, text="字幕视频")
        ...
        # 生成按钮
        self.generate_btn = ttk.Button(button_frame, text="生成字幕视频", command=self.generate_subtitle_video, width=12)
            ...
            for block in blocks:
                ...
                if len(lines) >= 3:
                    ...
                    if start_seconds >= video_duration:
                        print(f"跳过超出视频时长的字幕: {text}")
                        skipped_count += 1
                        continue
                        
                    if end_seconds > video_duration:
                        print(f"截断超出视频时长的字幕: {text}")
    ...
    def generate_subtitle_video(self):
        ...
        try:
            ...
            print("5. 构建FFmpeg命令...")
            # 构建基础FFmpeg命令
            # 使用原始路径，不进行转义（FFmpeg可以正确处理Windows路径）
            subtitle_path_clean = subtitle_path
            ...
            try:
                ...
                # 使用Windows原生路径格式，不进行转换
                subtitle_path_native = subtitle_path
                
                # 使用正确的字幕滤镜格式，参考您提供的正确命令
                # 路径需要转义冒号，使用单引号包围路径和样式
                subtitle_path_escaped = subtitle_path_native.replace(':', '\\:')
        ...
        try:
            ...
            if sys.platform == 'win32':
                ...
                process = subprocess.Popen(
                    ...
                    creationflags=subprocess.CREATE_NEW_CONSOLE,  # 给ffmpeg分配独立控制台
                    ...
                    if line:
                        ...
                        if 'time=' in line:
                            try:
                                ...
                                progress = min(progress, 95)  # 限制最大进度为95%
            ...
            if os.path.exists(output_path):
                ...
                # 检查文件完整性
                if self.check_video_file_integrity(output_path):
                    print("文件完整性检查通过")
                else:
                    print("警告：文件可能不完整或损坏")
            else:
            ...
            # 处理完成回调
            print("7. 调用完成回调...")
            self.root.after(0, self.handle_subtitle_completion, process.returncode, output_path)
            ...
            if process is not None:
                try:
                    ...
                    if process in self.active_processes:
                        self.active_processes.remove(process)
                        print(f"[DEBUG] 从活跃进程列表中移除字幕进程 PID: {process.pid}")
                    
                    # 确保进程已完全结束
                    if process.poll() is None:
                        print(f"[DEBUG] 字幕进程 {process.pid} 仍在运行，等待其结束...")
                        try:
                            process.wait(timeout=5)
                        except subprocess.TimeoutExpired:
                            print(f"[DEBUG] 字幕进程 {process.pid} 超时，强制终止")
                            process.terminate()
                            try:
                                process.wait(timeout=2)
                            except subprocess.TimeoutExpired:
                    ...
                    print(f"[DEBUG] 字幕进程 {process.pid} 清理完成")
                except Exception as e:
    ...
    def enable_subtitle_buttons(self):
        """启用字幕相关按钮"""
        if hasattr(self, 'generate_btn'):
            self.generate_btn.config(text="生成字幕视频", state="normal")
        if hasattr(self, 'preview_btn'):
